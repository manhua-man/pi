...
## 8. Compaction 双实现深入（增量摘要合并 + split turn 边界）

> 🔧 源码归纳 · 两套平行实现：核心抽象层 `packages/agent/src/harness/`（harness v2 铺垫）与产品层 `packages/coding-agent/src/core/`（当前生效）。同源算法、细节分叉。

### 8.1 双实现差异速览

| 维度 | agent harness | coding-agent core |
|------|---------------|-------------------|
| LLM 调用 | `Models` 抽象 · `completeSimpleWithRetries` | apiKey/headers/env + streamFn · `completeSummarization` |
| 错误风格 | `Result<T,E>` + `CompactionError`（aborted 显式处理） | throw 异常（compaction 路径 aborted 未处理） |
| 上下文重建 | `retainedTail` 内嵌压缩条目 + `firstKeptEntryId` 兜底 | 仅 `firstKeptEntryId` |
| cut 点 token 累计 | 仅 message entry | 所有 context 消息（含 summary 文本） |
| turn start 判定 | message 只认 `user`/`bashExecution` | + `custom`/`branchSummary`/`compactionSummary` 角色 |
| 空摘要守卫 | 无 | 有（messages/turnPrefix 均空返回 undefined） |
| 缺 entry 处理 | throw `SessionError(invalid_session)` | 静默 break |

共同点：`DEFAULT_COMPACTION_SETTINGS = { enabled, reserveTokens: 16384, keepRecentTokens: 20000 }`、cut-point 搜索、SUMMARIZATION / UPDATE / BRANCH / TURN_PREFIX 四套 prompt、fileOps 追踪（`edited ∪ written`；`read − modified`）。

### 8.2 增量摘要的合并规则

- **窗口定位**：取最后一个 compaction entry（prevCompactionIndex）；`previousSummary = prevCompaction.summary`；`boundaryStart` = `prevCompaction.firstKeptEntryId` 在 pathEntries 的 findIndex（找不到回退 `prevCompactionIndex + 1`）。
- **prompt 切换**：有 previousSummary → `UPDATE_SUMMARIZATION_PROMPT`（PRESERVE 旧信息 / ADD 新 progress·decisions·context / Progress 中 In Progress → Done / Next Steps 更新 / 允许删除不再相关项）；无 → `SUMMARIZATION_PROMPT` 全量。
- **请求内容**：只送增量消息 `[boundaryStart, historyEnd)` + `<previous-summary>` 标签内的旧摘要，不重发全部历史。
- **链式累积**：新 summary 成为下一次 previousSummary。
- **fileOps 合并**：prevCompaction.details（`readFiles`→read、`modifiedFiles`→edited，仅非 fromHook）+ 逐条 `extractFileOpsFromMessage`；split turn 时 turnPrefixMessages 的 file ops 并入。
- **分支摘要嵌套**：`prepareBranchEntries` 第一遍收集所有 branch_summary entry 的 details（跨分支文件追踪），第二遍从新到旧装消息到 budget。

### 8.3 split turn 边界细节

切割落在非轮起点消息、且该轮 turn start 位于 `[startIndex, cutIndex)` 时触发，三区划分：

```text
boundaryStart ──── turnStartIndex ──── cutIndex ──── boundaryEnd
  messagesToSummarize │  turnPrefixMessages  │  retainedTail
```

- **两次独立 LLM 调用**：history 摘要（增量合并 previousSummary，maxTokens = 0.8 × reserveTokens）+ turn prefix 摘要（`TURN_PREFIX_SUMMARIZATION_PROMPT`，maxTokens = 0.5 × reserveTokens）；拼接 `historyText + "\n\n---\n\n**Turn Context (split turn):**\n\n" + prefixText`，usage 用 `combineUsage`。
- **不变量**：cut 永不在 toolResult（tool call 与其结果不分离）；cut 落在 assistant 消息上时其后的 toolResults 保留进 retainedTail。
- **退化路径**：turnStartIndex === cutIndex（cut 恰落在 branch_summary/custom_message entry）→ turnPrefixMessages 空 → 走普通单次摘要。
- **两套实现在此的实质差异**：message role === "custom" 时，agent 判定为 split（单独摘要前缀），coding-agent 视为轮起点整段入 messagesToSummarize。
